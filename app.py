...
def render_communications(user):
    ...
    with t1:
        with st.form("send_message_form", clear_on_submit=True):
            ...
            enviar_btn = st.form_submit_button("🔔 Guardar en App y Avisar por WhatsApp", type="primary", use_container_width=True)
            
        # Fuera del formulario, procesamos la acción
        if enviar_btn:
            if mensaje.strip():
                # 1. Guardamos el mensaje internamente en la base de datos
                ...
                st.success(f"✅ ¡Mensaje guardado en el buzón de la app para: {destino}!")
                
                # 2. Preparamos el botón automático de WhatsApp
                ...
                if roles[destino] == "owner":
                    # Si va para el dueño, usamos el número configurado
                    link = f"https://wa.me/{format_phone(APP_CONFIG['BOSS_PHONE'])}?text={urllib.parse.quote(texto_wa)}"
                else:
                    # Si va para empleados, abre WhatsApp para que elijas a quién mandárselo
                ...
                st.info("👇 Tu mensaje ya está en la app. Toca el botón verde para notificar a su teléfono.")
                st.markdown(f'<a href="{link}" target="_blank" class="btn-whatsapp" style="display:flex; justify-content:center; align-items:center;">{svg_whatsapp} ENVIAR AVISO POR WHATSAPP</a>', unsafe_allow_html=True)
